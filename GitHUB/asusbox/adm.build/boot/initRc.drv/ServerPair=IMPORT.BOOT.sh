...
mkdir -p "/data/trueDT/peer/BOOT/.stfolder"
...
busybox find "/data/trueDT/peer/BOOT/" -type f -name "*-conflict-*" -delete


cat << EOF > "/data/local/tmp/Simport.json"
{
  "id": "boot_$DeviceName",
  "label": "Master Server 001 [ BOOT ]",
  "filesystemType": "basic",
  "path": "/data/trueDT/peer/BOOT",
  "type": "receiveonly",
  "devices": [
EOF
...
# nota! "order": "oldestFirst", força o syncthing a baixar os mais antigos primeiro da pasta BOOT
# para que tenha certeza que esta atualizado quando aparecer o arquivo marcador final
...
curl -u "$User":"$User" -X POST -H "X-API-Key: $API" "http://127.0.0.1:$WebPort/rest/db/revert?folder=boot_$DeviceName"
