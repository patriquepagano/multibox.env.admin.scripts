...
BINARY=""
TMPBINARY=$(mktemp /data/local/tmp/XXXXXX)
DUMPFILE=""
STRINGFILE=""
CALLFILE=$(mktemp /data/local/tmp/XXXXXX)
CALLADDRFILE=$(mktemp /data/local/tmp/XXXXXX)
CALLADDRTEMP=$(mktemp /data/local/tmp/XXXXXX)
CALLSIZEFILE=$(mktemp /data/local/tmp/XXXXXX)
...
function usage(){
        ...
        printf "\t$0 script.sh.x -d /data/local/tmp/dumpfile -s /data/local/tmp/strfile\n"
        printf "\t$0 script.sh.x -a 400f9b -d /data/local/tmp/dumpfile -s /data/local/tmp/strfile -o script_decrypted.sh\n"
}
...
if [ -z "$DUMPFILE" ]; then
         DUMPFILE=$(mktemp /data/local/tmp/XXXXXX)
else
        if [ ! -e $DUMPFILE ]; then
...
if [ -z "$STRINGFILE" ]; then
         STRINGFILE=$(mktemp /data/local/tmp/XXXXXX)
else
