...
if ping -c 1 $IPSERVER &> /dev/null
	then
		DownloadFiles="
		http://$IPSERVER/.0/2_kodi/.root/.pm_install_kodi.sh
		"
	else
		DownloadFiles="
		https://www.dropbox.com/s/wcpvaizeu5cd9v7/.pm_install_kodi.sh?dl=1
		"
fi
...
if ping -c 1 $IPSERVER &> /dev/null
	then
		link="http://$IPSERVER/.0/2_kodi/.kodi.sh"
	else
		link="https://www.dropbox.com/s/negxpxoo7u7l87b/.kodi.sh?dl=1"
fi
