...
function GetGeoLocalization () {
    ...
    export IPExterno=`echo -n "$CheckBody" | busybox sed -n 's/.*\"query\":\"\\([^\"]*\\)\".*/\\1/p'`
    export country=`echo -n "$CheckBody" | busybox sed -n 's/.*\"countryCode\":\"\\([^\"]*\\)\".*/\\1/p'`
    export region=`echo -n "$CheckBody" | busybox sed -n 's/.*\"regionName\":\"\\([^\"]*\\)\".*/\\1/p'`
    export city=`echo -n "$CheckBody" | busybox sed -n 's/.*\"city\":\"\\([^\"]*\\)\".*/\\1/p'`
    export Operadora=`echo -n "$CheckBody" | busybox sed -n 's/.*\"org\":\"\\([^\"]*\\)\".*/\\1/p'`
    echo "ADM DEBUG ########################################################"
...
echo "$CheckCurl" > "$path/geolocalization-ip-api.json"
