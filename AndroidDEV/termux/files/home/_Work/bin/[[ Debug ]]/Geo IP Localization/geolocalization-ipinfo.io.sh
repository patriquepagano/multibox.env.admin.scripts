#!/system/bin/sh
clear
path="$( cd "${0%/*}" && pwd -P )"
parent_path="$(dirname "$path")"
# /data/trueDT/peer/Sync < paths oficiais
SERVICE_NAME="ipinfo"

function GetGeoLocalization () {
    # https://ipinfo.io/
    # https://iplocality.com/
    # https://ipapi.co/#pricing
    # https://ipregistry.co/
    link='https://ipinfo.io'
    # CheckCurl=`/system/bin/curl --silent -w "%{http_code}" -k -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.127 Safari/537.36" "$link" -L`
    CheckCurl=`/system/bin/curl -sS --cacert "/data/Curl_cacert.pem" -w "\n%{http_code}" "$link" -L`
    export httpCode=`echo "$CheckCurl" | busybox tail -n 1`
    CheckBody=`echo "$CheckCurl" | busybox sed '$d'`
    export IPExterno=`echo -n "$CheckBody" | busybox sed -n 's/.*\"ip\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p'`
    export country=`echo -n "$CheckBody" | busybox sed -n 's/.*\"country\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p'`
    export region=`echo -n "$CheckBody" | busybox sed -n 's/.*\"region\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p'`
    export city=`echo -n "$CheckBody" | busybox sed -n 's/.*\"city\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p'`
    export Operadora=`echo -n "$CheckBody" | busybox sed -n 's/.*\"org\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p'`
    echo "ADM DEBUG ########################################################"
    echo "ADM DEBUG ### IP GetGeoLocalization"
    echo "ADM DEBUG ### httpCode = $httpCode" 
    echo "ADM DEBUG ### $IPExterno"
    echo "ADM DEBUG ### $country"
    echo "ADM DEBUG ### $region"
    echo "ADM DEBUG ### $city"
    echo "ADM DEBUG ### $Operadora"
}

echo "ADM DEBUG ########################################################"
echo "ADM DEBUG ### call function > GetGeoLocalization"
GetGeoLocalization

echo "$CheckCurl" > "$path/geolocalization-ipinfo.io.json"

if [ ! "$1" == "skip" ]; then
    echo "Press any key to exit."
    read bah
fi
