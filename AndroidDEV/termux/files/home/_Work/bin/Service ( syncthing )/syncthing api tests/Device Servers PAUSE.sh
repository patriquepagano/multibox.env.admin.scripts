...
# When omitted, pauses all devices. Returns status 200 and no content upon success, 
...
echo "$DevicesPairing" | while read line; do
    ...
    echo "******* Pausando a sincronização > $DeviceN"
    # /rest/db/completion?device=I6KAH76-...-3PSROAU
    curl -X POST -H "X-API-Key: $API" "http://127.0.0.1:4442/rest/system/pause?device=$DeviceID"
done
