...
termux/files/usr/" > /storage/DevMount/AndroidDEV/.stignore
...
cat << EOF > "/data/local/tmp/Simport.json"
{
  ...
  "path": "/storage/DevMount/AndroidDEV",
  "type": "sendonly",
  "devices": [
EOF
...
curl -u "$User":"$User" -X POST -H "X-API-Key: $API" "http://127.0.0.1:$WebPort/rest/db/override?folder=op9v5-ktf27"
