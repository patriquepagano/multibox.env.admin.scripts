...
busybox blkid | busybox grep "MultiBOX"
MBR=`busybox blkid | busybox grep "MultiBOX" | busybox grep "/sd" | busybox cut -d ":" -f 1  | busybox rev | busybox cut -c2- | busybox rev`
publicM=`busybox blkid | busybox grep "MultiBOX" | busybox grep "public" | busybox cut -d ":" -f 2`
...
check=`busybox mount | busybox grep "/storage/MultiBOX"`
if [ ! "$check" == "" ]; then
	echo "ADM DEBUG ### precisa desmontar unidade > /storage/MultiBOX"
	/system/bin/busybox umount -f $MBR'1' > /dev/null 2>&1
	/system/bin/busybox umount -f "/storage/MultiBOX" > /dev/null 2>&1
fi
