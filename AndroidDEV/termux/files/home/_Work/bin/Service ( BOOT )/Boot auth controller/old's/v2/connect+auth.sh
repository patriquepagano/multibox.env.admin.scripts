...
# extract vars
Serial=`echo -n $CheckCurl | busybox cut -d ";" -f 1 | busybox sed 's/\r$//'`
Pin=`echo -n $CheckCurl | busybox cut -d ";" -f 2 | busybox sed 's/\r$//'`
Status=`echo -n $CheckCurl | busybox cut -d ";" -f 3 | busybox sed 's/\r$//'`
httpCode=`echo -n $CheckCurl | busybox cut -d ";" -f 4 | busybox cut -d " " -f 2 | busybox sed 's/\r$//'`
# echo "
# $CheckCurl
# "
# echo "ADM DEBUG ### Serial   : $Serial"
# echo "ADM DEBUG ### Pin      : $Pin"
# echo "ADM DEBUG ### Status   : $Status"
# echo "ADM DEBUG ### httpCode : $httpCode"
# exit
}

# ler o serial e pin
LocalSerial=`cat $PathSerial`
LocalPin=`cat $PathPin`
...
if [ ! -f $PathSerial ]; then
	/system/bin/busybox mount -o remount,rw /system
	echo -n $Serial > $PathSerial
fi
...
# esta é a situação quando uma box foi instalada, ficou em trial por um tempo, o servidor apagou a pasta remota e o cliente tentou utilizar
if [ "$Status" == "No user Homefolder" ]; then
	echo "ADM DEBUG ### arquivos local > $LocalSerial"
	echo "ADM DEBUG ### arquivos online > $Serial"
	/system/bin/busybox mount -o remount,rw /system
	echo -n $Serial > $PathSerial
	Serial=`cat $PathSerial`
	Pin=`cat $PathPin`
	echo "Acesso trial terminou pastas apagadas no servidor"
	echo " Rodar um script local para o cliente comprar o acesso
	seu serial	= $Serial 
	seu PIN		= $Pin"
	exit
fi

# terminou o trial do cliente
if [ "$Status" == "Seu teste gratuito expirou." ]; then
	export Serial=`cat $PathSerial`
	export Pin=`cat $PathPin`
	echo "Seu teste gratuito expirou.
	seu serial	= $Serial 
	seu PIN		= $Pin"
	exit
fi

if [ "$Status" == "Entre em contato com suporte." ]; then
	export Serial=`cat $PathSerial`
	export Pin=`cat $PathPin`
...
# depois de passar pelos filtros de log
/system/bin/busybox mount -o remount,rw /system
echo -n $Pin > $PathPin
